                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        def findbgr():
                            ...
                            data = np.reshape(cropped_img, (height * width, 3))
                            ...
                            bgr = cv2.cvtColor(cropped_img, cv2.COLOR_BGR2RGB)
                            return bgr
                        ...
                        def bgrtorgb(value):
                            colors, count = np.unique(
                                value.reshape(-1, value.shape[-1]), axis=0, return_counts=True)
                            return colors[count.argmax()]
                        
                        rgbvalcol = tuple(bgrtorgb(bgrval))
